  ...
  function SavedVanillaMR:writeBPs(pwDevData,anaDevData,cuda,bpfi)
    ...
    do
      ...
      for m = 2, numMents do
        ...
        local mention_path = "/m" .. tostring(m)
        pwrite(h5, mention_path .. "/pw_scores", scores)

        for i = 1, #self.pwNet.modules do
          local path = mention_path .. "/pw/" .. tostring(i)
          print(path)
        ...
        for i = 1, #self.naNet.modules do
          local path = mention_path .. "/na/" .. tostring(i)
          print(path)
          local output = self.naNet.modules[i].output
          print(type(output))
          if type(output) == 'table' then
            for k, v in pairs(output) do
              pwrite(h5, path .. "/" .. k, v)
